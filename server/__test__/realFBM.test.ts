...
describe('Real FBM Integration Test', () => {
    beforeAll(() => {
        // Ensure real network requests are allowed
        nock.cleanAll();           // Clear any mocks
        nock.enableNetConnect();   // Allow real HTTP
        nock.restore();            // Reset any internal nock state
      });
  ...
  it('adds a real guest to FoodBank Manager', async () => {
    const newGuest = {
      ...
      cf_guests_45ae5f86e4: "In Country",       // string; required; valid eligible service
      cf_guests_8e6f172090: "Small- 1 to 3",    // string; required; valid service size
      ...
      street_address: "Chum Bucket HQ",         // string; optional but recommended if homeless is 0
